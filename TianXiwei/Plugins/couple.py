import random
import asyncio
from TianXiwei.Database import couple_collection , waifu_collection
from datetime import datetime, timedelta
from pytz import timezone
from pyrogram import Client, filters
from pyrogram.types import Message
from TianXiwei.Database.couple_db import save_couple , save_waifu
from TianXiwei.Functions.gender import detect_gender
...
async def choose_couple(chat_id: int, members: list) -> tuple:
    """Choose two random members from the chat, ordered (male, female) when
    their genders can be confidently detected from their names - otherwise
    falls back to the original random order (never blocks/breaks on an
    unclear or undetectable name)."""
    humans = [member for member in members if not member['is_bot']]
    if len(humans) < 2:
        return None, None

    person_a = random.choice(humans)
    person_b = random.choice([m for m in humans if m != person_a])

    try:
        gender_a, prob_a = await detect_gender(person_a["first_name"])
        gender_b, prob_b = await detect_gender(person_b["first_name"])
    except Exception:
        gender_a, gender_b = None, None

    # Only reorder when we're reasonably confident and they're not both the
    # same detected gender (in which case there's no meaningful "male first"
    # to pick anyway).
