...
from TianXiwei.Extra.chatadmin import fetch_admin_privileges ,  chatadmin , can_pin_messages , can_delete_messages , can_promote_members
from pyrogram.enums import ChatMembersFilter , ChatMemberStatus , ChatType , ParseMode
from config import config as c
import time
from TianXiwei import app , admin_cache , admin_cache_reload , log
from pyrogram.errors import ChatAdminRequired, ChatInvalid , MessageDeleteForbidden , RPCError , UserNotParticipant
from TianXiwei.Functions.user import resolve_user , LOWPROMOTE , PROMOTE , FULLPROMOTE , UNMUTE , MUTE
from TianXiwei.Database.rules_db import get_rules , set_rules ,  clear_rules
...
async def invite_link(client: app, message: Message):  # type: ignore
    try:
        ...
        bot_username = getattr(c, "BOT_USERNAME", None) or client.me.username
        encoded = encode_chat_id(message.chat.id)
        deep_link = f"https://t.me/{bot_username}?start=req_{encoded}"

        share_button = InlineKeyboardMarkup(
            [[InlineKeyboardButton("🔗 𝖦𝖾𝗍 𝖫𝗂𝗇𝗄", url=deep_link)]]
        )

        await message.reply(
            f"𝖧𝖾𝗋𝖾 𝖨𝗌 𝖳𝗁𝖾 𝖯𝗋𝗂𝗏𝖺𝗍𝖾 𝖩𝗈𝗂𝗇 𝖫𝗂𝗇𝗄 𝖥𝗈𝗋 𝖳𝗁𝗂𝗌 𝖢𝗁𝖺𝗍:\n{deep_link}",
