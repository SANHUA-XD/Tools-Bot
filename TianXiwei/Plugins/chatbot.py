from TianXiwei import app , CHATBOT_GROUP
from TianXiwei.Database.chatbotdb import enable_chatbot , disable_chatbot , is_chatbot_enabled
...
async def handle_chatbot(client : Client , message : Message):
    ...
    if message.reply_to_message.from_user.id == config.BOT_ID:
        ...
        # (see TianXiwei/helper/ai_provider.py) so that if one provider is down
